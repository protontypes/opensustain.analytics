...
def process_organisations_data(df_organisations: pd.DataFrame):
    ...
    df_clean = df_organisations[
        df_organisations["location_country"].notna()
        & (df_organisations["location_country"].str.strip() != "")
    ].copy()
    ...
    df_projects_country = (
        df_clean.groupby("iso_alpha")["total_listed_projects_in_organization"]
        .sum()
        .reset_index()
    )
...
def render_organisations_tab(df_organisations):
    ...
    df_clean, df_countries_count, df_continent_counts, df_projects_country = (
        process_organisations_data(filtered_df_organisations)
    )
    ...
    with st.container(border=True):
        ...
        df_orgs_by_type = (
            filtered_df_organisations.assign(
                form_of_organization=filtered_df_organisations[
    ...
    with st.container(border=True):
        ...
        fig_top_org_countries = px.bar(
            df_countries_count.sort_values("count", ascending=False).head(
                top_n_orgs_countries
            ),
            x="count",
            y="iso_alpha",
            orientation="h",
        ...
        fig_top_org_countries.update_layout(
